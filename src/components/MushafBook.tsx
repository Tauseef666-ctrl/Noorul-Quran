...
/**
 * An opened-book Mushaf: a proper hardcover with a spine, two facing pages,
 * ornamental gold-sprigged borders, and the real quranic text set in
 * traditional right-aligned calligraphy. Reads right-to-left like a real
 * Mushaf — odd page on the right, even page on the left. Only real provider
 * data is rendered; per-ayah playback stays reachable and the currently playing
 * ayah gets a subtle highlight (the text itself is never animated).
 */
  ...
  return (
    <div className="relative flex-1">
      <div className="relative h-full overflow-hidden bg-[#f4ead2] shadow-[inset_0_0_40px_rgba(90,70,35,0.12)]">
        ...
        <div
          className={`pointer-events-none absolute inset-2 rounded-[6px] border border-[#b49b5f]/50 ${
            side === 'left' ? 'rounded-l-none' : 'rounded-r-none'
          }`}
        />
        <div
          className={`pointer-events-none absolute inset-[9px] border border-[#b49b5f]/25 ${
            side === 'left' ? 'rounded-l-none' : 'rounded-r-none'
          }`}
        />

        <div className="relative flex h-full flex-col px-5 py-8 sm:px-8 sm:py-10">
          {/* running page header */}
...
export function MushafBook({
  ...
  return (
    <motion.div
      ...
      initial={{ rotateY: direction * 12, opacity: 0.35 }}
      animate={{ rotateY: 0, opacity: 1 }}
      exit={{ rotateY: direction * -16, opacity: 0 }}
      transition={{ duration: 0.45, ease: [0.32, 0.72, 0, 1] }}
    >
      {/* opened hardcover book frame */}
